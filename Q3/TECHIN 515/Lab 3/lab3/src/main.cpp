...
void checkButtons() {
    ...
    while (!buttonPressed) {  // Keep displaying the question until a button is pressed
        ...
        if (digitalRead(BUTTON_A) == LOW) {
            responses[questionIndex] = 1;  // Store response as integer 1 for A
            buttonPressed = true;
            Serial.println("Button A pressed");
        } 
        else if (digitalRead(BUTTON_B) == LOW) {
            responses[questionIndex] = 2;  // Store response as integer 2 for B
            buttonPressed = true;
            Serial.println("Button B pressed");
        }
        else if (digitalRead(BUTTON_C) == LOW) {
            responses[questionIndex] = 3;  // Store response as integer 3 for C
            buttonPressed = true;
            Serial.println("Button C pressed");
        }
        else if (digitalRead(BUTTON_D) == LOW) {
            responses[questionIndex] = 4;  // Store response as integer 4 for D
            buttonPressed = true;
            Serial.println("Button D pressed");
        }
        
        // If a button was pressed, wait for the button to be released (debouncing)
        if (buttonPressed) {
            // Display which option was selected
            display.setCursor(0, 55);
            display.print("Selected: ");
            display.print(char('A' + responses[questionIndex] - 1));
            display.display();
            ...
            // Additional delay for debouncing
            delay(200);
        }
        
        delay(50);  // Small delay to prevent button bouncing during polling
    }
