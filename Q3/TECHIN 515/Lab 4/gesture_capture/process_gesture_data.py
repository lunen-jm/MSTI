...
try:
    # Force reinstall of pyserial during runtime
    # This fixes the issue where serial.Serial is not found
    import sys
    import subprocess
    import importlib
    
    # First try to import as normal
    import serial
    
    # Check if Serial attribute exists
    if not hasattr(serial, 'Serial'):
        print("Warning: serial.Serial not found. Attempting to fix...")
        # Try to reinstall pyserial
        subprocess.check_call([sys.executable, "-m", "pip", "uninstall", "-y", "serial"])
        subprocess.check_call([sys.executable, "-m", "pip", "uninstall", "-y", "pyserial"])
    ...
    # Now import serial.tools.list_ports
    import serial.tools.list_ports
    
except Exception as e:
    print(f"Error setting up serial module: {e}")
    print("Please make sure pyserial is correctly installed: pip install pyserial")
    print("If you have a package named 'serial', please uninstall it: pip uninstall serial")
    import sys
    sys.exit(1)
...
def main():
    ...
    try:
        # Verify that Serial class exists before trying to use it
        if not hasattr(serial, 'Serial'):
            raise AttributeError("module 'serial' has no attribute 'Serial'. "
                               "This is usually caused by having both 'pyserial' and 'serial' packages installed. "
                               "Run 'pip uninstall -y serial' and then 'pip install pyserial'")
        ...
        print("\nExiting...")
    except Exception as e:  # Using generic Exception to catch all errors
        print(f"Error: {e}")
